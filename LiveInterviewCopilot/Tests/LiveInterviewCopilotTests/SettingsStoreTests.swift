...
final class SettingsStoreTests: XCTestCase {
    ...
    private func makeStore(
        ...
        let suite = defaults ?? {
            let name = "com.jude864huang.liveinterviewcopilot.test.\(UUID().uuidString)"
    ...
    func testDefaultNotesTemplateIDRoundTripAndNormalizesGenericToNil() {
        let suiteName = "com.jude864huang.liveinterviewcopilot.test.\(UUID().uuidString)"
    ...
    func testDefaultSaveAudioRecording() {
        let store = makeStore()
        XCTAssertTrue(store.saveAudioRecording)
    }
