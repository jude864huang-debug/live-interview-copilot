...
final class KnowledgeBaseTests: XCTestCase {
    ...
    private func makeSettings() -> AppSettings {
        let suiteName = "com.jude864huang.liveinterviewcopilot.kbtest.\(UUID().uuidString)"
    ...
    // MARK: - TextSimilarity.interviewMatchScore

    func testInterviewMatchScorePrioritizesNormalizedExactMatch() {
        let score = TextSimilarity.interviewMatchScore(
            "How would you validate AI Agent 效果？",
            "how would you validate ai agent 效果"
        )
        XCTAssertEqual(score, 1.0, accuracy: 0.001)
    }

    func testInterviewMatchScoreSupportsChinesePhraseOverlap() {
        let related = TextSimilarity.interviewMatchScore(
            "你如何验证这个方案的效果？",
            "你如何验证效果？"
        )
        let unrelated = TextSimilarity.interviewMatchScore(
            "你如何验证这个方案的效果？",
            "最大的风险是什么？"
        )
        XCTAssertGreaterThanOrEqual(related, 0.55)
        XCTAssertLessThan(unrelated, related)
    }

    func testInterviewMatchScoreSupportsMixedLanguageKeywords() {
        let score = TextSimilarity.interviewMatchScore(
            "How do you validate AI Agent 效果?",
            "你会如何验证 AI agent 的效果？"
        )
        XCTAssertGreaterThanOrEqual(score, 0.55)
    }

    func testInterviewMatchScoreRecognizesChineseSemanticParaphrase() {
        let score = TextSimilarity.interviewMatchScore(
            "怎么判断这个项目最后做得好不好？",
            "你如何验证效果？"
        )
    ...
    func testInterviewMatchScoreRecognizesResourceParaphrase() {
        let score = TextSimilarity.interviewMatchScore(
            "如果预算砍半，你会怎么调整？",
            "资源继续减少怎么办？"
        )
    ...
    func testInterviewMatchScoreDoesNotConflateDifferentInterviewConcepts() {
        let score = TextSimilarity.interviewMatchScore(
            "如果预算砍半，你会怎么调整？",
            "你如何验证效果？"
        )
        XCTAssertLessThan(score, 0.55)
    }
