...
final class LiveSessionControllerTests: XCTestCase {
    ...
    private func makeTempDirs() -> (root: URL, notes: URL) {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("Live Interview CopilotLiveSessionTests", isDirectory: true)
    ...
    private func makeSettings(
        ...
        let suiteName = "com.jude864huang.liveinterviewcopilot.tests.livesession.\(UUID().uuidString)"
    ...
    private func installViewServices(
        ...
        let suiteName = "com.jude864huang.liveinterviewcopilot.tests.viewservices.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName) ?? .standard
        defaults.removePersistentDomain(forName: suiteName)
        let container = AppContainer(
            mode: .live,
    ...
    private func makeUninitializedController(
        ...
        let defaults = UserDefaults(suiteName: "com.jude864huang.liveinterviewcopilot.tests.lazyservices.\(UUID().uuidString)") ?? .standard
        let container = AppContainer(
    ...
    func testAudioRetentionPlanExportsWhenUserKeepsInterviewRecordingEnabled() {
        let dirs = makeTempDirs()
        let settings = makeSettings(notesDirectory: dirs.notes)
        ...
        XCTAssertTrue(plan.shouldExportRecording)
    ...
    func testInterviewRecordingPreferenceDefaultsOnOnceAndThenRespectsUserChoice() {
        ...
        AppContainer.initializeInterviewRecordingPreference(defaults: defaults)
        XCTAssertTrue(defaults.bool(forKey: "saveAudioRecording"))

        defaults.set(false, forKey: "saveAudioRecording")
        AppContainer.initializeInterviewRecordingPreference(defaults: defaults)
        XCTAssertFalse(defaults.bool(forKey: "saveAudioRecording"))
    }
