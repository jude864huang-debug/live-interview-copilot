...
final class NotesControllerTests: XCTestCase {
    ...
    private func makeTempDirs() -> (root: URL, notes: URL) {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("Live Interview CopilotNotesControllerTests", isDirectory: true)
    ...
    private func makeSettings(notesDirectory: URL) -> AppSettings {
        let suiteName = "com.jude864huang.liveinterviewcopilot.tests.notescontroller.\(UUID().uuidString)"
    ...
    func testSessionSourceGroupsKeepGranolaSeparateFromLiveInterviewCopilot() async {
        let (root, _) = makeTempDirs()
        let (controller, coordinator) = makeController(root: root)
        let base = Date(timeIntervalSince1970: 1_700_000_000)
        ...
        XCTAssertEqual(groups.map(\.title), ["Granola", "Live Interview Copilot"])
