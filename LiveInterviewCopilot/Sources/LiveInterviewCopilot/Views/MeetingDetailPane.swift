...
struct MeetingDetailPane<SessionFolderMenuItems: View>: View {
            ...
            VStack(spacing: 0) {
                ...
                if showsBottomSection {
                    ...
                    ContentUnavailableView(
                        ...
                        description: Text("Live Interview Copilot hasn’t saved any other meetings for this title yet.")
                    )
    ...
    private func startRecording(for event: CalendarEvent, selectedTemplate: MeetingTemplate?) {
        ...
        openWindow(id: LiveInterviewCopilotRootApp.mainWindowID)
    ...
    private func audioPlaybackButton(controller: NotesController, state: NotesState) -> some View {
        ...
        Menu {
            ForEach(sources) { source in
                ...
                let actionTitle = state.isPlayingAudio && isSelectedSource
                    ? "暂停\(audioSourceTitle(source))"
                    : "播放\(audioSourceTitle(source))"
            ...
            Button {
                ...
                Label("在 Finder 中显示录音", systemImage: "folder")
            }
        } label: {
            Label(
                state.isPlayingAudio ? "暂停面试录音" : "播放面试录音",
        ...
        .buttonStyle(.bordered)
        .fixedSize()
        .help(state.isPlayingAudio ? "暂停本场面试录音" : "播放本场面试录音（麦克风与系统音频）")
    }

    private func audioSourceTitle(_ source: SessionAudioSource) -> String {
        switch source.kind {
        case .recording:
            return "面试录音（麦克风 + 系统音频）"
        case .system:
            return "系统音频"
        case .microphone:
            return "麦克风音频"
    ...
    private func appleNotesSyncButton(controller: NotesController, state: NotesState) -> some View {
        ...
        } ?? "Export these notes to Apple Notes. The note will be created in your \"\(settings.appleNotesFolderName.isEmpty ? "Live Interview Copilot" : settings.appleNotesFolderName)\" folder.")
    }
    ...
    private func askMessageBubble(_ message: TranscriptChatMessage) -> some View {
        ...
        return HStack {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(isUser ? "You" : "Live Interview Copilot")
                    .font(.system(size: 10, weight: .semibold))
                    .foregroundStyle(.secondary)
    ...
    private func emptyTranscriptMessage(
        ...
        var message = issue?.emptyStateMessage ?? "Live Interview Copilot does not have a transcript for this session."
        if recoveryIsPending {
                ...
                LazyVStack(alignment: .leading, spacing: 14) {
                    ...
                    ForEach(Array(state.loadedInterviewAnswers.enumerated()), id: \.element.id) { index, record in
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            if let suggestions = record.followUpSuggestions {
                                Divider()
                                Text("可能追问与预判答案")
                                    .font(.system(size: 11, weight: .bold))
                                    .foregroundStyle(Color.accentColor)
                                ForEach(Array(suggestions.items.enumerated()), id: \.element.id) { followUpIndex, suggestion in
                                    persistedFollowUpView(
                                        index: followUpIndex,
                                        suggestion: suggestion,
                                        answer: record.followUpAnswers?[suggestion.question]
                                    )
                                }
                            }

                            if let predictedQuestion = record.predictedFollowUpQuestion,
                               let predictedAnswer = record.predictedFollowUpAnswer {
                                Divider()
                                VStack(alignment: .leading, spacing: 5) {
                                    Text("本轮命中的预判参考")
                                        .font(.system(size: 11, weight: .bold))
    ...
    private func persistedFollowUpView(
        index: Int,
    ...
    private func formattedInterviewAIAnswers(_ records: [InterviewHistoryAnswer]) -> String {
        records.enumerated().map { index, record in
            ...
            let followUps = record.followUpSuggestions?.items.enumerated().map { followUpIndex, suggestion in
                let answerText: String
                if let answer = record.followUpAnswers?[suggestion.question] {
                    let points = answer.talkingPoints.enumerated().map {
                        "\($0.offset + 1). \($0.element)"
                    }.joined(separator: "\n")
                    answerText = "\(answer.directOpening)\n\n\(points)\n\n\(answer.sampleAnswer)"
                } else {
                    answerText = "答案未完成"
                }
                return "### Q\(followUpIndex + 1)：\(suggestion.question)\n\n\(answerText)"
            }.joined(separator: "\n\n") ?? ""
            let followUpSection = followUps.isEmpty ? "" : "\n\n## 可能追问与预判答案\n\n\(followUps)"
            return "## 问题 \(index + 1)：\(record.question)\n\n\(segments)\(followUpSection)"
        }.joined(separator: "\n\n")
    }
    ...
    private func formattedAskConversation() -> String {
        askMessages.map { message in
            let label = message.role == .user ? "You" : "Live Interview Copilot"
