...
final class AppContainer {
    ...
    private static let interviewRecordingPreferenceInitializedKey =
        "interviewRecordingPreferenceInitialized"
    ...
    static func bootstrap() -> AppLaunchContext {
        ...
        switch mode {
        case .live:
            initializeInterviewRecordingPreference(defaults: .standard)
            let container = AppContainer(
                ...
                appSupportDirectory: FileManager.default.urls(
                    ...
                    in: .userDomainMask
                ).first!.appendingPathComponent("Live Interview Copilot", isDirectory: true),
        ...
        case .uiTest(let scenario):
            let runID = environment["LIVE_INTERVIEW_COPILOT_UI_TEST_RUN_ID"] ?? UUID().uuidString
            let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("LiveInterviewCopilotUITests", isDirectory: true)
            ...
            let suiteName = "com.jude864huang.liveinterviewcopilot.uitests.\(runID)"
    ...
    static func initializeInterviewRecordingPreference(defaults: UserDefaults) {
        guard !defaults.bool(forKey: interviewRecordingPreferenceInitializedKey) else { return }
        defaults.set(true, forKey: "saveAudioRecording")
        defaults.set(true, forKey: interviewRecordingPreferenceInitializedKey)
    }
    ...
    func makeViewServices(settings: AppSettings, coordinator: AppCoordinator) -> AppViewServices {
        ...
        let customerCopilotEngine = CustomerCopilotEngine(
            ...
            interviewAnswerLoadHandler: { sessionID in
                await sessionRepository.loadInterviewAnswers(sessionID: sessionID)
            },
            defaults: defaults
        )
    ...
    private static func runtimeMode(from environment: [String: String]) -> AppRuntimeMode {
        guard environment["LIVE_INTERVIEW_COPILOT_UI_TEST"] == "1" else {
            return .live
        }

        let scenario = UITestScenario(rawValue: environment["LIVE_INTERVIEW_COPILOT_UI_SCENARIO"] ?? "")
            ?? .launchSmoke
