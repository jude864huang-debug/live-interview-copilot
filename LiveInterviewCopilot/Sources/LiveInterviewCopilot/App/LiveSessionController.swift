...
final class LiveSessionController {
    ...
    /// 1. `LIVE_INTERVIEW_COPILOT_SILENCE_TIMEOUT_SECONDS` env override (for testing),
    ...
    static func effectiveAutomaticSilenceTimeoutInterval(settings: AppSettings?) -> TimeInterval {
        if let raw = ProcessInfo.processInfo.environment["LIVE_INTERVIEW_COPILOT_SILENCE_TIMEOUT_SECONDS"],
    ...
    func startTranscription(metadata: MeetingMetadata, settings: AppSettings?) async {
        ...
        state.isLocalRecordingEnabled = false
        if let settings {
            let audioRetentionPlan = Self.audioRetentionPlan(settings: settings, utteranceCount: nil)
            state.isLocalRecordingEnabled = audioRetentionPlan.shouldExportRecording
            if audioRetentionPlan.shouldStartRecorder {
                if let recorder = coordinator.audioRecorder {
                    let recordingDirectory = await coordinator.sessionRepository
                        .prepareRecordingDirectory(sessionID: handle.sessionID)
                    recorder.updateDirectory(recordingDirectory)
    ...
    func finalizeCurrentSession(settings: AppSettings?) async {
        ...
        await coordinator.customerCopilotEngine?.awaitPendingInterviewHistoryPersistence()
    ...
    static func audioRetentionPlan(settings: AppSettings, utteranceCount: Int?) -> AudioRetentionPlan {
        ...
        let shouldStartRecorder = shouldExportRecording
            || shouldRetainBatchAudio
            || settings.transcriptionModel.isCloud
        return AudioRetentionPlan(
