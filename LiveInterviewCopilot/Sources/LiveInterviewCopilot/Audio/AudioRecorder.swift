...
final class AudioRecorder: @unchecked Sendable {
    ...
    func startSession() {
        lock.withLock {
            ...
            micTempURL = tmp.appendingPathComponent("liveinterviewcopilot_mic_\(sessionTimestamp).caf")
            sysTempURL = tmp.appendingPathComponent("liveinterviewcopilot_sys_\(sessionTimestamp).caf")
        }
    }
    ...
    private func mergeAndEncode() {
        ...
        do {
            ...
            Log.recorder.error("Failed to create recording directory: \(error, privacy: .public)")
            return
        }
