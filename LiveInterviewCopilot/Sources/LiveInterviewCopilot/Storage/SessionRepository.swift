...
actor SessionRepository {
    ...
    /// User-facing notes folder for mirroring (e.g. ~/Documents/LiveInterviewCopilot).
    ...
    init(rootDirectory: URL? = nil) {
        ...
        if let rootDirectory {
            ...
            baseDirectory = appSupport.appendingPathComponent("Live Interview Copilot", isDirectory: true)
        }
    ...
    /// Retains the latest validated snapshot for each interview round. Reusing
    /// a round ID replaces that record as follow-up answers are completed.
    ...
    func exportPlainText(sessionID: String) -> String {
        ...
        var result = "LiveInterviewCopilot - \(headerFmt.string(from: startDate))\n\n"
    ...
    /// Returns the canonical local directory for a session's recording.
    /// Keeping the exported M4A here makes it discoverable by session history.
    func prepareRecordingDirectory(sessionID: String) -> URL {
        ...
        do {
            ...
            reportWriteError("Failed to create recording directory: \(error.localizedDescription)")
        }
        return directory
    }
