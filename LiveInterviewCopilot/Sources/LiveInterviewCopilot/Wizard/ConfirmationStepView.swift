...
struct ConfirmationStepView: View {
    ...
    private var narrativeText: some View {
        ...
        VStack(spacing: 8) {
            if isTranscriptOnly {
                Text("You're all set. When your next call starts, LiveInterviewCopilot will notice and ask if you'd like to record. Say yes, and it transcribes every word of the conversation in real time.")
                    .font(.system(size: 13))
                    .multilineTextAlignment(.center)
                    .lineSpacing(3)
            } else {
                Text("You're all set. When your next Zoom, Meet, or Teams call starts, LiveInterviewCopilot will notice and ask if you'd like to record. Say yes, and it transcribes the conversation and generates notes with action items when you're done.")
                    .font(.system(size: 13))
                    .multilineTextAlignment(.center)
                    .lineSpacing(3)
            }
    ...
    private var micPermissionSection: some View {
        ...
        case .notDetermined:
            VStack(spacing: 8) {
                Text("One last thing: LiveInterviewCopilot needs microphone access to transcribe your meetings.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
        ...
        case .denied, .restricted:
            VStack(spacing: 8) {
                Text("Microphone access was previously denied. LiveInterviewCopilot needs it to transcribe.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
