...
struct ProviderSetupStepView: View {
    ...
    private var cloudSetupContent: some View {
        VStack(spacing: 20) {
            ...
            Text("To use cloud AI, you'll need an API key. This is like a password that connects LiveInterviewCopilot to the AI service.")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            if viewModel.intent == .fullCopilot {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("Only needed if you want LiveInterviewCopilot to search a Knowledge Base folder for relevant context during meetings. This does not improve note generation directly.")
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
    ...
    private var ollamaSetupContent: some View {
        VStack(spacing: 20) {
            ...
            Text("To run AI on your Mac, LiveInterviewCopilot uses Ollama. Let's check if it's ready.")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
