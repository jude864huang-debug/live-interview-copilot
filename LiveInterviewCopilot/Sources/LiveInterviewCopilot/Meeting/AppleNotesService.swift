...
/// `[LiveInterviewCopilot] YYYY-MM-DD <meeting title>` so re-exports always overwrite the
...
enum AppleNotesService {
    ...
    static func noteTitle(for sessionIndex: SessionIndex) -> String {
        ...
        if let meetingTitle, !meetingTitle.isEmpty {
            return "[LiveInterviewCopilot] \(dateStr) \(meetingTitle)"
        }
        return "[LiveInterviewCopilot] \(dateStr) Meeting"
    }
    ...
    private static func runExport(accountName: String, folderName: String, title: String, html: String, existingNoteID: String?) async -> String? {
        let tempURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("liveinterviewcopilot-apple-notes-export.html")
        ...
        // AppleScript interprets backslash sequences in string literals,
        // and we need to prevent double-quotes from breaking out of the string.
        let safeAccount  = accountName.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "\"", with: "'")
        let safeFolder   = folderName.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "\"", with: "'")
        ...
        let safeNoteID   = (existingNoteID ?? "").replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "\"", with: "'")
    ...
    private static func resolvedFolderName(_ raw: String) -> String {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? "Live Interview Copilot" : trimmed
    }
    ...
    private static func buildHTML(
        ...
        body += "<h2>\(htmlEscaped(title))</h2>\n"
        body += "<p><i>(Auto-generated by LiveInterviewCopilot. Manual edits made here will be lost if the note is regenerated in the app.)</i></p>\n"
        ...
        if includeTranscript, !transcriptEntries.isEmpty {
            ...
            for entry in transcriptEntries {
                ...
                body += "<p>[\(ts)] <b>\(htmlEscaped(entry.speaker)):</b> \(htmlEscaped(entry.text))</p>\n"
            }
        }
