...
final class ParakeetBackend: TranscriptionBackend, @unchecked Sendable {
    ...
    func checkStatus() -> BackendStatus {
        let exists = AsrModels.modelsExist(
            at: LiveInterviewCopilotLocalModelStore.parakeetDirectory(for: version),
            version: version
        )
    ...
    func clearModelCache() {
        LiveInterviewCopilotLocalModelStore.clearParakeetCache(for: version)
    }

    func prepare(onStatus: @Sendable (String) -> Void, onProgress: @escaping @Sendable (Double) -> Void) async throws {
        let models: AsrModels
        let modelsDirectory = LiveInterviewCopilotLocalModelStore.parakeetDirectory(for: version)
