...
struct DiagnosticsReportBuilder {
    ...
    static func buildText(
        ...
        var lines: [String] = []
        lines.append("Live Interview Copilot Diagnostics Export")
...
actor DiagnosticsBreadcrumbStore {
    ...
    private func breadcrumbsURL() -> URL {
        ...
        return appSupport
            .appendingPathComponent("Live Interview Copilot", isDirectory: true)
...
enum DiagnosticsSupport {
    ...
    private static func defaultFilename() -> String {
        ...
        return "liveinterviewcopilot-diagnostics-\(formatter.string(from: Date())).txt"
    }

    @MainActor
    private static func generateReport(settings: SettingsStore) async -> String {
        let bundle = Bundle.main
        let bundleID = bundle.bundleIdentifier ?? "com.jude864huang.liveinterviewcopilot.app"
    ...
    private static func unifiedLogReport(bundleIdentifier: String) async -> String {
        await Task.detached(priority: .utility) {
            let subsystems = Set([bundleIdentifier, "com.jude864huang.liveinterviewcopilot.app"]).sorted()
